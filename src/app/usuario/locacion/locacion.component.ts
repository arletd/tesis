...
export class LocacionComponent implements OnInit {
  ...
  async getTipo(){
    this.tipo = await  this.dbAcc.read('eventos/'+this.actual+'/tipo');
  }

  async getInformacion(){
    var totalOb = await this.dbAcc.read('propuestas/'+this.actual+'/locacion/total');
    var total;
    if(totalOb== null){
      total = 0;
    }
    ...
    console.log(total);
    var auxFotos = await this.dbAcc.readJSON('propuestas/'+this.actual+'/locacion/'+total+'/fotos');
    this.opcion1 = await this.dbAcc.readJSON('propuestas/'+this.actual+'/locacion/'+total);
    ...
    total=total-1;
    var auxFotos2 = await this.dbAcc.readJSON('propuestas/'+this.actual+'/locacion/'+total+'/fotos');
    this.opcion2 = await this.dbAcc.readJSON('propuestas/'+this.actual+'/locacion/'+total);
    ...
    total=total-1;
    var auxFotos3 = await this.dbAcc.readJSON('propuestas/'+this.actual+'/locacion/'+total+'/fotos');
    this.opcion3 = await this.dbAcc.readJSON('propuestas/'+this.actual+'/locacion/'+total);
  ...
  cargaInformacion(){
    ...
    var ddl = (document.getElementById("opcionLocacion") as HTMLSelectElement);
  ...
  guardarEleccion(){
    var elecccion = (document.getElementById("opcionLocacion") as HTMLSelectElement).value;
    this.dbAcc.write('/eventos/'+this.actual+'/decisiones/locacion',elecccion);
